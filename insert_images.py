...
def get_next_image(output_file):
    ...
    occurrences.wait()
    last_image = subprocess.Popen(
        ("tail", "-n", "1"), stdin=occurrences.stdout, stdout=subprocess.PIPE
    )
    last_image = last_image.stdout.read().decode("utf-8")
    image_number = re.findall("[0-9]+", last_image)
    if len(image_number) == 0:
        return 1
    assert len(image_number) == 1, "Error with: {image_number}"
